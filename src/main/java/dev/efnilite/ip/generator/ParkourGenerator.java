...
public class ParkourGenerator {
    ...
    public void tick() {
        ...
        // Trim fully-faded blocks from the front of `history` so long runs don't
        // retain an ever-growing list of Block references (each holds a World ref).
        // Index 0 (the spawn block) is preserved because reset() depends on it.
        int retainTail = 4 * BLOCK_TRAIL + 4;
        if (currentIndex > retainTail) {
            int removeCount = currentIndex - retainTail;
            history.subList(1, 1 + removeCount).clear();
            currentIndex = retainTail;
